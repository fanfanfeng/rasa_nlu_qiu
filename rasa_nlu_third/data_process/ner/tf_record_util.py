...
from rasa_nlu_third.utils.ner_data_utils import pad_sentence, pad_sentence_rasa
...
def make_tfrecord_files(params):
    ...
    if not os.path.exists(tfrecord_save_path):
        ...
        if params.data_type == 'default':
            ...
            for char_and_label in data_processer.load_folder_data(data_processer.train_folder):
                input_ids, label_ids, segment_ids, input_mask = pad_sentence_rasa(char_and_label,params.max_sentence_length, vocab, labels_ids)
                feature_item = tf.train.Example(features=tf.train.Features(feature={
                    'input_ids': _int64_feature(input_ids, need_list=False),
                    'label_ids': _int64_feature(label_ids, need_list=False),
                    'segment_ids': _int64_feature(segment_ids, need_list=False),
                ...
                tfrecord_train_writer.write(feature_item.SerializeToString())
            for char_and_labels in data_processer.load_folder_data(data_processer.test_folder):
                input_ids, label_ids, segment_ids, input_mask = pad_sentence_rasa(char_and_labels,params.max_sentence_length, vocab, labels_ids)
                feature_item = tf.train.Example(features=tf.train.Features(feature={
                    'input_ids': _int64_feature(input_ids, need_list=False),
                    'label_ids': _int64_feature(label_ids, need_list=False),
                    'segment_ids': _int64_feature(segment_ids, need_list=False),
