...
class Meta_Load():
    def __init__(self,model_dir,use_bert=False,max_sentence_len=None):
    ...
    def predict(self,text):
        ...
        tokens = np.array(self.pad_sentence(words,self.max_sentence_len,self.vocabulary_dict)).reshape((1,self.max_sentence_len))
        text_leng = len(words)
        tokens_mask = []
        ...
        tokens_mask = np.array(tokens_mask).reshape((1,self.max_sentence_len))
